...
import {
  BadgeCheck, Wrench, Headphones, Star, Play,
...
/* ───────────────────────  SUPPORT QUICK LINKS  ─────────────────────── */
const quickLinks = [
  { icon: BadgeCheck, label: 'Where to buy', href: '/where-to-buy' },
  { icon: Wrench, label: 'Install', href: '/services' },
      ...
      <section className="bg-[#f7f7f7] pt-6 pb-24" data-nav-theme="light">
        <div className="max-w-[1100px] mx-auto px-6 grid grid-cols-3 gap-8">
          {quickLinks.map((q) => {
            const Icon = q.icon;
            return (
              <Link key={q.label} href={q.href} className="group flex flex-col items-center text-center">
                <Icon className="w-[52px] h-[52px] text-[var(--finevu-orange)]" strokeWidth={1.6} />
                <p className="text-[#0b0b0c] font-bold text-[20px] mt-5">{q.label}</p>
                <span className="text-[#5c6478] text-[15px] mt-2 group-hover:text-[var(--finevu-orange)] transition-colors inline-flex items-center gap-1">
