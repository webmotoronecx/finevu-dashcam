...
const troubleshooting = [
    {
        q: 'Camera says "check memory card" or footage won’t save',
        a: "Format the card in the camera’s own menu (not on a computer), and make sure you’re using a high-endurance card rated for dash cams. Cards should be re-formatted every 1–2 months for reliable recording.",
    },
    {
        q: "Phone won’t connect to the camera’s Wi-Fi",
        a: "Turn off mobile data while connecting, ensure you’re joining the camera’s own Wi-Fi network (FINEVU_xxxx), and check the camera is in Wi-Fi pairing mode. If it still fails, forget the network on your phone and pair again.",
    },
    {
    ...
    },
    {
        ...
        a: "Allow up to 5 minutes for first GPS lock in open sky. Windscreens with metallic tint or heating elements can block signal — try repositioning the camera or GPS antenna closer to a non-tinted area.",
    },
    {
    ...
    },
    {
        ...
        a: "Set your time zone (GMT offset) in the camera settings — with GPS connected, time syncs automatically once the zone is correct. Remember to adjust for daylight saving if your region uses it.",
    },
];
                ...
                {items.map(([label, meta, url]) => (
                    <li key={label}>
                        ...
                        >
                            ...
                            <span className="shrink-0 font-medium text-[#9c9ca3]">{url ? meta : "Coming Soon"}</span>
                        </a>
                    </li>
                ))}
            </ul>
        ...
        >
            ...
            <div className="px-8 py-9 sm:px-11">
                ...
                <div className="mt-9 flex flex-wrap items-center gap-4 border-t border-[#e7e7ea] pt-7">
                    <a
                        href="#register"
                        className="cta-hover rounded-full bg-[var(--finevu-orange)] px-7 py-3.5 text-[14px] font-semibold uppercase leading-[20px] text-white"
                    >
                        Register this product
                    </a>
                    <Link
                        href="/contact"
                        className="rounded-full border border-[#1d1d1f] px-7 py-3.5 text-[14px] font-semibold uppercase leading-[20px] text-[#1d1d1f] transition-colors hover:bg-[#1d1d1f] hover:text-white"
                    >
                        Contact support
                    </Link>
                        ...
                        <Fragment key={m.id}>
                            <button
                                ...
                                className={`flex flex-col items-center rounded-[28px] bg-white px-8 pb-8 pt-9 text-center transition-colors ${
                                    selected ? "border-2 border-[var(--finevu-orange)]" : "border border-[#bbbbbb] hover:border-[var(--finevu-orange)]"
                                }`}
                            >
                                ...
                                <span
                                    className={`inline-flex items-center gap-1.5 rounded-full px-6 py-2.5 text-[15px] font-semibold ${
                                        selected ? "bg-[var(--finevu-orange)] text-white" : "border border-[var(--finevu-orange)] text-[var(--finevu-orange)]"
                                    }`}
                                >
                ...
                <div className="mx-auto max-w-[1160px] px-6">
                    <motion.div {...fadeUp} className="mx-auto mb-11 max-w-[720px] text-center">
                        ...
                        <p className="mt-4 text-[18px] leading-[1.6] text-[#6b6b72]">
                            Get in touch with our team, or find manuals, firmware and troubleshooting for your FineVu dash cam below.
                        </p>
                    </motion.div>
                    ...
                    <div className="mx-auto grid max-w-[900px] gap-[22px] md:grid-cols-2">
                        {regPanels.map((p, i) => (
                            <motion.div
                                ...
                                {...fadeUp}
                                transition={{ duration: 0.5, delay: i * 0.08 }}
                                className="flex flex-col rounded-[16px] border border-[#e7e7ea] bg-white p-[30px]"
                            >
