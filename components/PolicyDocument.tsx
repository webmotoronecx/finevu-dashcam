...
export function PolicyDocument({ title, meta, sections }: PolicyDocumentProps) {
    ...
    const [expanded, setExpanded] = useState<Set<number>>(() => new Set(sections.map((s, i) => (s.subsections?.length ? i : -1)).filter((i) => i >= 0)));
        ...
        <div className="min-h-screen bg-white" data-nav-theme="light">
            ...
            <div className="mx-auto grid max-w-[1180px] gap-10 px-6 pb-24 lg:grid-cols-[300px_1fr] lg:gap-16 lg:px-10">
                ...
                <nav className="lg:sticky lg:top-28  lg:self-start">
                    <div className="scrollbar-minimal rounded-2xl border border-[#e8e7e2] p-3 lg:max-h-[600px] lg:overflow-y-auto relative overflow-hidden">
                    <ol className="">
                        {sections.map((section, i) => {
                            ...
                            return (
                                <li key={section.n}>
                                    <div className={`flex items-start rounded-lg transition-colors ${isActive ? "bg-[#fff1e8]" : "hover:bg-[#f7f6f3]"}`}>
                                        <button
                                            type="button"
                                            onClick={() => select(i)}
                                            className="flex flex-1  items-start gap-3 px-3 py-2 text-left text-[14px] leading-snug"
                                        >
                                            <span className={`shrink-0 tabular-nums pt-[2px] text-[11px] w-[16px] text-right ${isActive ? "text-[var(--finevu-orange)]" : "text-[#9a9da5]"}`}>
                                                {section.n}.
                                            </span>
                                            <span className={isActive ? "font-medium text-[#1d1d1f]" : "text-[#3a3a40]"}>{section.title}</span>
                                        </button>
                                        {hasSubs && (
                                            <button
                                                type="button"
                                    ...
                                    {hasSubs && isOpen && (
                                        <ul className="mb-1 ml-[36px] space-y-1 border-l border-[#e8e7e2] pl-3">
                ...
                <article ref={contentRef} className="scroll-mt-28">
                    <h2 className="mb-6 text-[21px] font-semibold text-[#1d1d1f] flex flex-row gap-2 items-center">
                        <span className=" text-[14px] flex items-end justify-start text-[#9a9da5]">{current.n}.</span>
                        {current.title}
                    </h2>
